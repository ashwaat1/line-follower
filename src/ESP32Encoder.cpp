...
void ESP32Encoder::attach(int a, int b, encType et) {
	...
	if(! attachedInterrupt){
		...
		if (isrServiceCpuCore == ISR_CORE_USE_DEFAULT || isrServiceCpuCore == xPortGetCoreID()) {
			...
			esp_ipc_call_blocking(isrServiceCpuCore, ipc_install_isr_on_core, &ipc_ret_code);
		}
#endif
...
void ESP32Encoder::setCount(int_fast32_t value) {
	_ENTER_CRITICAL();
	count = value - getCountRaw();
	_EXIT_CRITICAL();
}

int_fast32_t ESP32Encoder::getCountRaw() {
	int16_t c;
...
int_fast32_t ESP32Encoder::getCount() {
	_ENTER_CRITICAL();
	int_fast32_t result = count + getCountRaw();
	_EXIT_CRITICAL();
	return result;
}

int_fast32_t ESP32Encoder::clearCount() {
	_ENTER_CRITICAL();
	count = 0;
	_EXIT_CRITICAL();
...
int_fast32_t ESP32Encoder::pauseCount() {
	return pcnt_counter_pause(unit);
}

int_fast32_t ESP32Encoder::resumeCount() {
